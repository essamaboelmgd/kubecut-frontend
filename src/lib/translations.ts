...
export const unitTypeLabels: Record<string, string> = {
     ...
     tall_drawers_side_appliances_doors: 'دولاب ادراج م جانبية + اجهزة + ضلف علوية',
...
export const getEdgeMarks = (code: string | undefined) => {
     ...
     // General Rule: Any "M" code puts a groove on the RIGHT side (يمين)
     if (code.includes("M")) {
          marks.right = groove_mark;
     }

     if (code.includes("OM")) {
          marks.top = tape_mark; marks.bottom = tape_mark; marks.left = tape_mark;
          // Right is Groove
